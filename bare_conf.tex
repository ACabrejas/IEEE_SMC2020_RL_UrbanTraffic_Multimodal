...
However, most of the existing works restrict themselves to vehicles only, not attempting to jointly optimise vehicular and pedestrian travel times, even though pedestrians are allowed and present in the great majority of real urban intersections.
...
Recent research suggests that more complex state representations only provide marginal gains, if any\cite{gendersstate}, hence the second approach is adopted in this paper.
Recently, extensive reviews are being carried out \cite{mannion} \cite{survey2020} \cite{survey2020wei} indexing the various approaches taken and pushing the field towards maturity but without comparing performances of the different approaches and proposed rewards.
...
\SetAlgoLined
 ...
 \For{each episode}{ 
    ...
    $b \leftarrow$ sample minibatch of transitions tuples from $M$\;
...
\bibitem{previous} Cabrejas Egea, A., Howell, S., Knutins, M., \& Connaughton C. (2020, October). Assessment of Reward Functions for Reinforcement Learning Traffic Signal Control under Real-World Limitations. IEEE Systems, Man, and Cybernetics (October 2020) (Accepted). 
