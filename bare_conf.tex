...
The use of Reinforcement Learning (RL) for Urban Traffic Control has been previously explored and discussed in a variety of pieces of research, aiming to substitute existing adaptive control methods such as SCOOT\cite{scoot}, MOVA\cite{mova} and SCATS\cite{scats}.
The field has evolved from early inquiries about its theoretical potential use \cite{wiering2000} \cite{abdul2003} \cite{abdoos2011}, to more applied and realistic scenarios \cite{survey2014} \cite{liang2017} \cite{genders2019} that look towards real-world use and deployment.
...
Recently, extensive reviews are being carried out \cite{survey2020} \cite{survey2020wei}, indexing the different approaches taken and pushing the field towards maturity.

A common thread in most of the previous works is the need for approximations about the network being studied and the lack of pedestrian modelling, joint optimisation for vehicles and pedestrians' travel times or the effect, and RL traffic control in the latter.
As indicated in \cite{survey2020}, pedestrian implementation has a high impact on learning performance, being often discarded as unimportant or left for future work save for two exceptions \cite{geneticped} \cite{liu2017} \cite{chen2020}, one of which uses a genetic algorithm.
...
\label{eq:reward}
\end{equation}
In this case, $\mathcal{T}$ is unknown, only allowing for model-free RL).
...
\begin{equation}
R_t = \alpha * R^p_t + \beta * R^v_t;\,\,\,\,\,\,\ \alpha+\beta = 1
\label{eq:modal}
\end{equation}
...
As such, series identified as P80 and P95 represent those in which the weights were $\alpha=0.8$, $\beta=0.2$, and $\alpha=0.95$, $\beta=0.05$ respectively. 
Those series without an identifier did not require of modal prioritisation.
...
In the results, it can be observed that while in general the choice (5,95) seems to be working best, for certain functional choices the prioritisation(20,80) produces best results, which would not be the case if the suitability of the weights was only affected by the relative demand ratios between vehicles and pedestrians (e.g. Throughput rewards).
