...
\hl{However, literature on the introduction and control of pedestrians to such intersections is scarce. %, and comparisons between the effect of the different approaches that can be taken in terms of architecture, state space or rewards are uncommon.
...
Some rewards proposed in previous literature in the context of only vehicular traffic are extended to pedestrians while others are introduced here.}
...
The assessed rewards can be broadly classified in 5 groups depending on the magnitudes used: queues, waiting time, delay, average speed and throughput in the junction.
...
We find that those rewards maximising the speed of the network obtain the lowest waiting time for vehicles and pedestrians simultaneously, closely followed by queue minimisation, demonstrating better performance than other methods previously proposed in the literature.
\end{abstract}
...
This has allowed the development of novel smart control approaches, harnessing the power of real-time data to deliver cheap and responsive systems that can adapt to a variety of situations.

Reinforcement Learning (RL) approaches have been showing promising results in this field.
\hl{However, most of the existing works restrict themselves to vehicles only, not attempting to jointly optimise vehicular and pedestrian travel times, even though pedestrians are allowed and present in the great majority of real urban intersections.}

This paper compares the performance of \hl{30 different reward functions used by Deep Q-Network agents}, split into 5 different classes based on the magnitudes they use, when controlling a simulation of a real-world junction in Greater Manchester (UK) that has been calibrated using 3.5 months of data gathered from Vivacity Labs vision-based sensors.
...
These use different methods, such as complex pixel-based inputs passed to a CNN \cite{liang2017}  \cite{gao2017} \cite{mousavi2017}, per-lane state signals using fully connected neural networks \cite{survey2014} \cite{aslani2019} \cite{genders2019}\hl{, or hybrid approaches }\cite{genders2016} \cite{gendersthesis}  \cite{wan2018}.
Recent research suggests that more complex state representations only provide marginal gains, if any\cite{gendersstate} \hl{, hence the second approach is adopted in this paper.}
Recently, extensive reviews are being carried out \cite{mannion} \cite{survey2020} \cite{survey2020wei} \hl{indexing the various approaches taken and pushing the field towards maturity but without comparing performances of the different approaches and proposed rewards.}
...
As indicated in one recent review \cite{survey2020}, pedestrian implementation has a high impact on learning performance, being often discarded as unimportant or left for future work save for three exceptions \cite{geneticped} \cite{liu2017} \cite{chen2020}, the first of which uses a genetic algorithm instead of RL.
In this paper we attempt to cover this gap in the literature, providing a robust performance assessment of RL agents serving both vehicles and pedestrians, using a variety of rewards, both novel and from the literature, \hl{attempting to uncover what state variables should be used in the reward to obtain the best performance.}
These are used in a calibrated model of a real-world junction, using real geometry, calibrated demand, realistic sensor inputs and emulated traffic light controllers, having deployed some of these agents to control real traffic since these experiments took place.
This paper delivers \hl{the future work deferred from }\cite{previous} \hl{in terms of shifting the focus towards pedestrians and multi-objective optimisation, while keeping the problem} grounded in the real world.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{algorithm}
\SetAlgoLined
 \hl{Create main network with random weights $\theta$}\;
 \hl{Create target network with random weights $\theta^-$}\;
 \hl{Create replay memory $M$ with capacity $L$}\;
 \hl{Define frequency $F$ for copying weights to target network}\;
 \For{each episode}{ 
    \hl{measure initial state $s_0$}\;
    \While{episode not done}{
        \hl{select action $a_t$ according to $\epsilon$-greedy policy}\;
        \hl{implement $a_t$}\;
        \hl{advance simulation until next action is needed}\;
        \hl{measure new state $s_{t+1}$, and calculate reward $R_{t+1}$}\;
        \hl{store transition tuple $(s_t,a_t,R_{t+1},s_{t+1})$ in $M$}\;
        \hl{$s \leftarrow s_{t+1}$}\;
        }
   \hl{ $b \leftarrow$ sample minibatch of transitions tuples from $M$}\;
    \For{each transition $x_i = (s_i,a_i,R_{i+1},s_{i+1})$ in $b$}{
        \hl{$y_i = R_{i+1} + \gamma \max_{a} Q(s_{i+1}, a', \theta^-)$}
        }
    \hl{Stochastic Gradient Descent on $\theta$ over all $(x_i,y_i) \in b$}\;
    \If{number of episode is multiple of $F$}{
        \hl{$\theta^- \leftarrow \theta$}
        }
    }
...
\bibitem{mannion} Mannion, P., Duggan, J., \& Howley, E. (2016). An experimental review of reinforcement learning algorithms for adaptive traffic signal control. Autonomic road transport support systems (pp. 47-66). Birkhäuser, Cham.
\bibitem{survey2020} Haydari, A., \& Yilmaz, Y. (2020). Deep Reinforcement Learning for Intelligent Transportation Systems: A Survey. Preprint arXiv:2005.00935.
\bibitem{survey2020wei} Wei, H., Zheng, G., Gayah, V., \& Li, Z. (2019). A Survey on Traffic Signal Control Methods. Preprint arXiv:1904.08117.
...
\bibitem{previous} Cabrejas Egea, A., Howell, S., Knutins, M., \& Connaughton C. (2020, October). Assessment of Reward Functions for Reinforcement Learning Traffic Signal Control under Real-World Limitations. IEEE Systems, Man, and Cybernetics (Accepted). 
