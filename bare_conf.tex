...
using a scalar reward signal as indication of the performance of its actions.
An optimal policy will be that which maximises the expected future discounted rewards $R_t$, choosing an optimal sequence of actions $\pi^*(s)=a^*$ to achieve it.
The parameter regulating how far into the future is the agent optimising for, is the discount factor $\gamma$, as shown in Eq. \ref{eq:reward}.


\subsection{Deep Reinforcement Learning}
The concept of Deep Reinforcement Learning refers to RL algorithms in which a Deep Neural Network is used to approximate the policy function.
...
\caption{Schematic representation of information transmission occurring between Agent and SUMO Environment}                                  
\label{rl}                                               
\end{figure}  

\subsection{Reinforcement Learning Agent}
The basic common agent used to obtain these results is a standard implementation of a Deep Q Network (DQN) in PyTorch \cite{pytorch}, optimising its weights via Stochastic Gradient Descent \cite{kiefer} using ADAM \cite{adam} as optimizer.
The learning rate is set to $\alpha=10^{-5}$ and the discount factor to $\gamma = 0.8$ for all simulations.
...
Our environment is modelled in the microscopic traffic simulator SUMO \cite{sumo}, representing a real-world intersection in Greater Manchester, UK.
...
\section{Modal Prioritisation}
The agent has to serve both vehicles and pedestrians arriving at the intersection, seeking to jointly optimise the intersection for both modes of transport.
